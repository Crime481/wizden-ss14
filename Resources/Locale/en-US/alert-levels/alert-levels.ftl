...
alert-level-green-announcement = There are no threats on the station. It is safe to return to your standard duties.
alert-level-green-instructions = Do your job.

alert-level-blue = Blue
alert-level-blue-announcement = There is an unconfirmed threat to the station. Security should perform random checks. Crewmembers are advised to be vigilant and report suspicious activity to security.
...
alert-level-amber-announcement = There is a confirmed threat to the station. Security should prepare to use lethal force if necessary. Crewmembers should find a safe place to shelter in, and are advised to follow any present authorities.
...
alert-level-delta-instructions = Evacuate from the station immediately.

alert-level-epsilon = Epsilon
alert-level-epsilon-announcement = Central Command has terminated all contracts with the station. Thank you for choosing NanoTrasen!
